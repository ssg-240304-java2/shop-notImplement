package com.ohgiraffers.notimplement.delivery.paging;

public class DeliveryPagenation {
    /* 검색어가 없는 경우 페이징 처리만을 위한 용도 */
    public static SelectDeliveryCriteria getSelectCriteria(int pageNo, int totalCount, int limit, int buttonAmount) {

        return getSelectCriteria(pageNo, totalCount, limit, buttonAmount, null, null);
    }

    /* 검색어가 존재하는 경우 검색 조건으로 select 후 페이징 처리를 하기 위한 용도 */
    public static SelectDeliveryCriteria getSelectCriteria(int pageNo, int totalCount, int limit, int buttonAmount, String searchCondition, String searchValue) {
...
//     startPage = (((int)((double) pageNo / buttonAmount + 0.9)) - 1) * buttonAmount + 1;
        ...
        SelectDeliveryCriteria selectCriteria = new SelectDeliveryCriteria(pageNo, totalCount, limit, buttonAmount, maxPage, startPage, endPage, startRow, endRow, searchCondition, searchValue);
